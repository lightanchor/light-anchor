...
History contains one release-style subject (`轻锚 0.1.0 · Iroha`), so no general convention is established. Use concise, imperative subjects and focused commits. Pull requests should explain the user-visible effect, note architectural or data-format changes, list verification commands, and link relevant issues. Include screenshots for UI changes and update both localization tables when copy changes. Never commit `.build/`, `dist/`, credentials, signing keys, or real user data.
